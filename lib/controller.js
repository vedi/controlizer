...
const ControlizerScope = require('./scope');
const checkRequired = require('./helpers/checkRequired');

const ERROR_TYPE = 'controlizer';
...
class Controller {
  ...
  createScope(controller, transport) {
    return new ControlizerScope(this, transport, this.contextFactory);
  }
          ...
          action.method.forEach((method) => {
            action.transports.forEach(transport =>
              transport.addRoute(this, method, this.path, action, async (scope) => {
                try {
                  const handlerResult = action.handler(scope);
                  const data = util.types.isPromise(handlerResult)
                    ? await handlerResult
                    : handlerResult;
  ...
  sendResult(scope) {
    scope.transport.sendResult(scope.controlizerResult, scope);
  }
